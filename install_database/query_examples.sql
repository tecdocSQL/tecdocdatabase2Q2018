/*
	поиск ремня ГРМ на FORD FUSION (JU_) 1.4
*/
...
-- FORD FUSION (JU_) 1.4 id = 16624, internalID = 20321
...
-- поиск internalID продукта "Комплект ремней ГРМ"
...
-- "Комплект ремней ГРМ" product_id = 260 и 2594
...
FROM (
	...
	WHERE
		...
		AND al.item = 20321 -- internalID из запроса к таблице passengercars
		AND al.product IN (260, 2594) -- internalID из предыдущего запроса
) ar
...
LEFT JOIN products p
    ON p.ID = a.CurrentProduct
WHERE
    acl.article_id = 41648 -- id искомого артикула
	AND a.IsValid = 1
    AND p.Description = "Тормозной диск" -- искомая товарная группа
ORDER BY Supplier
